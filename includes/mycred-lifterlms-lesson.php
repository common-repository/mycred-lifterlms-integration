 ...
 */
if ( ! class_exists( 'myCRED_LifterLMS_Lesson' ) ) :
	class myCRED_LifterLMS_Lesson extends myCRED_Hook {
		...
		public function run() {
			...
			if ( $this->prefs['creds'] !== 0 ) {
				add_action('lifterlms_lesson_completed', array($this, 'lesson_completed'), 50, 2 );
			}
		}

		/**
		 * Runs when a student completes a lesson
		 *
		 * @since 1.0
		 *
		 * @param $student_id
		...
		public function lesson_completed($student_id, $lesson_id) {
			...
			$refrence = 'lifterlms_lesson_completed';

			$data = ['lesson' => $lesson_id];

			// Enforce limit and make sure users only get points once per unique lesson
			if (! $this->over_hook_limit( '', $refrence, $student_id) && ! $this->core->has_entry($refrence, $student_id, $lesson_id, $data, $this->mycred_type)) {
