 ...
 */
if ( ! class_exists( 'myCRED_LifterLMS_Section' ) ) :
	class myCRED_LifterLMS_Section extends myCRED_Hook {
		...
		public function run() {
			...
			if ($this->prefs['creds'] !== 0) {
				add_action('lifterlms_section_completed', array($this, 'section_completed'), 100, 2);
			}
		}

		/**
		 * Runs when a student completes a section
		 *
		 * @since 1.0
		 *
		 * @param $student_id
		 * @param $section_id
		 */
		public function section_completed($student_id, $section_id) {
			// Check for exclusion
			if ($this->core->exclude_user($student_id)) return;

			$refrence = 'lifterlms_section_completed';

			$data = ['section' => $section_id];

			// Enforce limit and make sure users only get points once per unique section
			if (! $this->over_hook_limit('', $refrence, $student_id) && ! $this->core->has_entry($refrence, $student_id, $section_id, $data, $this->mycred_type)) {
				$this->core->add_creds(
					$refrence,
					$student_id,
					$this->prefs['creds'],
