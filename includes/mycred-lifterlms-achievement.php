 ...
 */
if ( ! class_exists( 'myCRED_LifterLMS_Achievement' ) ) :
	class myCRED_LifterLMS_Achievement extends myCRED_Hook {
		...
		function __construct($hook_prefs, $type = MYCRED_DEFAULT_TYPE_KEY) {
			...
			parent::__construct( array(
				'id'       => 'mycred_lifterlms_achievement',
				'defaults' => array(
					'creds'  => 1,
					'log'    => '%plural% for earning a new achievement',
					'limit'  => '0/x'
				)
			),$hook_prefs, $type);
		}
		...
		public function run() {
			...
			if ($this->prefs['creds'] !== 0) {
				add_action('llms_user_earned_achievement', array($this, 'user_earned_achievement'), 100, 3);
			}
		}

		/**
		 * Runs when a student earns new achievement
		 *
		 * @since 1.0
		 *
		 * @param $student_id
		 ...
		 */
		public function user_earned_achievement($student_id, $achievement_id, $related_post_id) {
			// Check for exclusion
			if ($this->core->exclude_user($student_id)) return;

			$refrence = 'llms_user_earned_achievement';
			...
			// Enforce limit and make sure users only get points once per unique achivement
			if ( ! $this->over_hook_limit('', $refrence, $student_id) && ! $this->core->has_entry($refrence, $student_id, $achievement_id, $data, $this->mycred_type)) {
				$this->core->add_creds(
					$refrence,
					$student_id,
					$this->prefs['creds'],
